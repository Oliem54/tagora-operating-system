...
```text
STATUS=LOT_2_DOCUMENT_INSTANTIATION_COMPLETE
...
Revalidation distante du 2026-08-12 : la branche `docs/tos-phase-4d-time` n'était plus au SHA de base attendu `2960a73e` ; tip observé `754baaf`. Les cinq scénarios QA Lot 2 étaient déjà présents (`ab227da`, puis alignement statut et handoff).

Martin reconnaît l'instanciation documentaire comme déjà réalisée. Aucune réécriture Time n'est requise. La base `2960a73e` est supersédée par l'historique distant observé pour la clôture documentaire Lot 2.
...
LOT_3_AUTHORIZED=NO
TIME_WRITE_REQUIRED=NO
TOS_CLOSURE_REQUIRED=YES
PHASE_4D=IN_PROGRESS
```

Fichiers Time reconnus (déjà présents, non modifiés ici) :
...
`COMPLETE` = documentation Lot 2 instanciée uniquement. Exécution QA terrain des scénarios : **NOT_STARTED**. Playwright : non autorisé. Lot 3 : non autorisé.

Le gate d'exécution QA est préparé séparément : [PREPARATION_PHASE_4D_LOT2_QA_EXECUTION_GATE_TIME.md](PREPARATION_PHASE_4D_LOT2_QA_EXECUTION_GATE_TIME.md) (VALD-094). Ce document-ci ne l'autorise pas.
